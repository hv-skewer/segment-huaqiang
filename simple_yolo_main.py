...
from simple_yolo_processor import SimpleYOLOVideoProcessor
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='华强买瓜视频分割 - 简化YOLO版本')
    ...
    processor = SimpleYOLOVideoProcessor(
    ...
    if args.test:
        ...
        logger.info("运行测试模式...")
        try:
            from simple_yolo_processor import test_yolo_model
            test_yolo_model()
            logger.info("测试完成")
            return 0
